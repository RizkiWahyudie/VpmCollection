...
<body onload="window.print()">
  ...
  $total_transaksi = $total['total'] + $total['ongkir'] - ($total['potongan_transaksi']);
  if ($total['proses'] == '0') {
  ...
  echo "<h2 style='margin:0px'><center>$iden[nama_website]<center></h2>
    ...
    <div class='col-md-4'>
        <center>
        Total Tagihan 
        <h3 style='margin:0px;'>Rp " . rupiah($total_transaksi) . "<br> <br>
...
<tr class='success'>
  ...
  echo
    "
                ...
                <tr class='success'>
                  <td colspan='4'><b>Ongkir</b> <i style='float:right'>(" . terbilang($total['ongkir']) . ")</i></td>
  ...
  <table width='50%' border=1 id='tablemodul1'>
    ...
    <tbody>
      ...
      foreach ($rekening->result_array() as $row) {
        echo "<tr><td>$no</td>
              <td>$row[nama_bank]</td>
              <td><a target='_blank' href='$row[no_rekening]'>$row[no_rekening]</a></td>
