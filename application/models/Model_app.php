...
class Model_app extends CI_model
{
    ...
    public function view_reseller($session)
    {
        return $this->db->query("SELECT * FROM `reseller_akun` where reseller_akun.id_konsumen = '$session'");
    }

    public function jml_transaksi($session)
    {
        return $this->db->query("SELECT * FROM `rb_penjualan` a where a.id_pembeli='$session'");
    }
    ...
    function cari_produk($kata)
    {
        ...
        $this->db->where('rb_produk.status_sale', 1);
    ...
    // buat permission role admin apa aja yang di perbolehkan akses menunya
    ...
    function konfirm_reseller()
    {
        return $this->db->query("SELECT * FROM `reseller_akun` a ORDER BY a.id DESC");
    }

    // function status_reseller() {
    //     $session = $this->session->id_konsumen;
    //     return $this->db->query("SELECT * FROM `reseller_akun` a where a.id_konsumen='$session'");
    // }
    ...
    {
        return $this->db->query("SELECT a.id, a.ktp, a.nama_olshop, a.link_olshop, a.status FROM `reseller_akun` a where a.id_konsumen='$id'");
    }
}