...
class Produk extends CI_Controller
{
	...
	{
		$this->load->database();
		$jumlah = $this->db->query("SELECT * FROM `rb_produk` a where a.status_sale=1")->num_rows();
		...
		if (is_numeric($dari)) {
			...
			if ($this->input->post('cari') != '') {
				...
				$data['record'] = $this->db->query("SELECT * FROM `rb_produk` a where a.status_sale=1 ORDER BY total_terjual DESC LIMIT $dari, $config[per_page]");
	...
	function kategori()
	{
		...
		if (is_numeric($dari)) {
			...
			$data['record'] = $this->db->query("SELECT * FROM `rb_produk` a where a.status_sale=1 AND a.id_kategori_produk=$cek[id_kategori_produk] ORDER BY total_terjual DESC LIMIT $dari, $config[per_page]");
			$this->pagination->initialize($config);
			$this->template->load('phpmu-one/template', 'phpmu-one/view_home', $data);
		} else {
			redirect('main');
			...
			if ($stok < $this->input->post('jumlah') or $stok <= '0') {
				...
				if ($cek >= 1) {
					...
					$data = array(
						...
						'id_produk' => $id_produk,
						'jumlah' => $jumlah,
						'harga_jual' => $harga['harga_reseller'],
						'satuan' => $harga['satuan'],
		...
		if (isset($_POST['submit'])) {
			if ($this->session->idp != '') {
				...
				$data = array(
					...
					'potongan_transaksi' => $this->input->post('disctransaksi'),
				...
				foreach ($keranjang->result_array() as $row) {
					$jumlahAwal = $this->db->query("SELECT * FROM `rb_produk` a WHERE a.id_produk=$row[id_produk]");
					$dataa = array(
						'id_penjualan' => $idp,
					...
					foreach ($jumlahAwal->result_array() as $hasilAwal) {
						$this->db->update('rb_produk', array('total_terjual' => $hasilAwal['total_terjual'] + $row['jumlah']), array('id_produk' => $row['id_produk']));
					}
				}
				...
				$data['total_bayar'] = rupiah($this->input->post('total') + $this->input->post('ongkir') - $this->input->post('disctransaksi'));
				...
				foreach ($rekening->result_array() as $row) {
					$message .= "<tr bgcolor='#e3e3e3'><td>$noo</td>
						              <td>$row[nama_bank]</td>
						              <td><a href='$row[no_rekening]'>$row[no_rekening]</a></td>
			...
			if ($this->session->id_konsumen) {
				...
				if ($cek->num_rows() >= 1) {
					...
					$data['reseller'] = $this->model_app->view_reseller($this->session->id_konsumen)->row_array();
					$data['record'] = $this->model_app->view_join_rows('rb_penjualan_temp', 'rb_produk', 'id_produk', array('session' => $this->session->idp), 'id_penjualan_detail', 'ASC');
					$data['jml_transaksi'] = $this->model_app->jml_transaksi($this->session->id_konsumen)->num_rows();
	...
	function wishlist()
	{
		...
		$data['record'] = $this->db->query("SELECT b.*, a.id_wishlist FROM `rb_wishlist` a INNER JOIN `rb_produk` b ON a.id_produk=b.id_produk where a.id_konsumen='$session'");
		$data['title'] = 'Wishlist';
		$this->template->load('phpmu-one/template', 'phpmu-one/pengunjung/view_wishlist', $data);
	}
	...
	function wishlist_add()
	{
		...
		$this->load->database();
		$this->db->insert('rb_wishlist', $data);
	...
	function wishlist_delete()
	{
		cek_session_members();
		$data = array(
			'id_wishlist' => $this->uri->segment(3)
		);
		$this->load->database();
		$this->db->delete('rb_wishlist', $data);
		redirect('/produk/wishlist');
	}

	function wishlist_delete_page()
	{
		cek_session_members();
		$data = array(
			'id_wishlist' => $this->uri->segment(3)
